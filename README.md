...
- 美化「我要回家」港鐵班次：大學站預計到達時間改為低干擾藍色資訊帶。
- 強化資訊層級：突出大學站及到達時間，車程改為簡潔「約 X 分鐘」。
- 加入班次分隔、深色模式及窄螢幕適配。
- 更新前端及 Service Worker cache 版本至 6.8.5。

## v6.8.4

- 「我要回家」港鐵即時班次新增「預計到達大學站」時間。
- 以每班車的即時開車時間加上所選東鐵站至大學站的預計車程計算；短途車如不經大學站則不顯示。
- 更新前端及 Service Worker cache 版本至 6.8.4。

## v6.8.3

- 巴士／小巴搜尋放大鏡浮球的拖動範圍同時受頂部標題及底部導覽列限制，不可再拖到兩者之上。
- 設定齒輪固定於頂部標題列右側，不再與「最後更新」文字重疊。
- 所有「更新中」提示取消 0%／50%／75% 等百分比，只保留簡潔的更新狀態文字。

## v6.8.2

- 底部分頁列改用單色 SVG 圖示：淺色模式黑色、深色模式白色；被選取分頁只以藍色圖示／文字表示，不再顯示藍色橢圓底。
- 頂部標題及設定圖示加入 iOS WebKit scroll-edge 防蒙糊處理；齒輪亦改用清晰單色 SVG。
- 巴士／小巴搜尋浮球拖動範圍會按實際底部分頁列位置動態限制，不能再拖入或被 navigation bar 遮住。
- 開啟 iOS 原生鍵盤時隱藏底部分頁列，避免 visual viewport 縮細後分頁列向上頂住搜尋欄。
...
### v6.9.2
- 「我要回家」港鐵班次改為 `1分鐘　23:33 → 大學 23:56 | 約 23 分鐘　往 上水 [7]` 的掃讀次序；「往 + 目的地 + 月台」整組靠右。
- 每班會經過大學站的港鐵列車均顯示預計到達時間及「約 X 分鐘」車程。
- 「我要回家」九巴 272A 改為 `1分鐘　23:33　預定班次`，並將原有「原定班次」字樣改為「預定班次」。
- 九巴班次之間加入與港鐵相同的灰色分隔線。
- 更新前端及 Service Worker cache 版本至 6.9.2。


### v6.9.0
- 「我要回家」九巴 272A 的開車時間取消括號，例如 `(23:40)` 改為 `23:40`。
- 九巴開車時間欄與上方港鐵 `23:44` 等開車時間共用同一水平欄位。
- `原定班次` 移至與港鐵 `→ 大學 00:10` 相同的資訊欄起點，令上下兩張卡的視覺掃讀一致。

### v6.8.9
- 「我要回家」港鐵開車時間由 4 位數字改回標準 `HH:MM`（例如 `23:33`）。
- 調整單行欄距：月台格至開車時間的視覺留白，與「分鐘 ETA」至「往目的地」的留白更一致。
- MTR 班次字體層級改為跟九巴 ETA 列表一致：分鐘、時間及一般文字採用相同的常規字重與比例；大學到達時間仍以港鐵藍作重點。

### v6.8.8
- 精修「我要回家」港鐵單行版面：ETA、目的地、月台、開車時間、到大學時間改用固定欄位對齊，連續班次更易掃視。
- 月台牌改為更細緻的港鐵藍漸層與輕陰影；分隔線、字重、數字間距及到站資訊層級同步微調。
- 保留 `| 約 X 分鐘` 只在第一班真正經過大學站的列車顯示；功能邏輯不變。

### v6.8.7
- 「我要回家」港鐵班次改為單行閱讀：`1分鐘  往 上水  [7]  2313 → 大學 23:36 | 約 23 分鐘`。
- 月台號碼改為港鐵藍實色長方格、白字顯示。
- 開車時間改為 4 位數字（例如 `2313`），到大學時間仍保留 `23:36`。
- `| 約 X 分鐘` 只顯示在第一班真正會經過大學站的列車；後續列車只顯示各自的預計到達大學時間。

### v6.8.6
- 「我要回家」港鐵班次改為單行旅程式排版：`往 1分鐘 上水 (7) 23:13 → 大學 23:36 | 約 23 分鐘`。
- 取消到達大學的獨立藍色資訊框；以港鐵藍突出「→ 大學 + 預計到達時間」，其餘資訊維持低干擾層級。
- 窄屏幕不足一行時才自動換行。
...
- 「我要回家」900 的「往白石角」方向標籤固定靠最右。
- 900 車站下拉只顯示主站名，隱藏括號及括號內附註。
- 900 標題恢復「九巴 900 -」。
- 「白石角出發」28A／28S ETA 改用與主要卡片一致的三欄排版。
- 更新前端及 Service Worker cache 版本至 6.9.7。

### v6.9.6

- 「白石角出發」與「我要回家」的 272A ETA 改用固定三欄排版，令「預定班次／最後班次」垂直對齊。
- 「我要回家」港鐵北面（往金鐘）下拉次序改為：落馬洲、羅湖、上水、粉嶺、太和、大埔墟。
- 「我要回家」在 272A 下新增九巴 900 往白石角，預設太古廣場，並可由官方 route-stop 資料動態選擇沿途車站；同時合併 service type 1/2 ETA。

### v6.9.5
- 修正「白石角出發」v6.9.4 CSS 尾段被誤寫成 literal `\n`，令瀏覽器忽略 outbound 單行排版的問題。
- 博研路 KMB 272A 現與「我要回家」272A 共用完全一致的 44px 單行節奏、欄位間距、垂直置中及灰色分隔線。
- 同步修復「白石角出發」大學站 MTR 單行版面，並加入窄屏幕對應。
- 前端、Service Worker 及 asset query string 升級至 6.9.5，避免已安裝 PWA 繼續讀取舊的壞 CSS。

### v6.9.4
- 將 v6.9.3 的 MTR／KMB 單行班次介面移植至「白石角出發」。
- 大學站 MTR 班次改為「分鐘｜開車時間｜右側往目的地＋月台」並沿用 44px 行高及灰色分隔線。
- 博研路 KMB 272A 改為「分鐘｜開車時間｜預定／最後班次」，與「我要回家」使用同一排版、垂直置中及行距。
- 更新前端及 Service Worker cache 版本至 6.9.4。

### v6.9.3
- KMB「我要回家」班次列改為垂直置中，並保持與 MTR 相同 44px 行高及灰色分隔線。
- KMB「最後班次」改到與「預定班次」相同的行內狀態欄位顯示。
- MTR `| 約 XX 分鐘` 只在第一個實際可到大學的班次顯示；其後可到大學班次只顯示預計到達時間。
- 更新前端及 Service Worker cache 版本至 6.9.3。
